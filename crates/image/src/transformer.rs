...
use napi_ohos::{bindgen_prelude::*, JsBuffer};
use resvg::{
    tiny_skia,
        ...
        match image {
            None => {
                ...
                let dynamic_image = if image_format == ImageFormat::Avif {
                    ...
                    if (width * height * 3) as usize == decoded_length {
                        ImageBuffer::from_raw(width, height, decoded_rgb)
                            ...
                            .ok_or_else(|| {
                                Error::new(
                                    Status::InvalidArg,
                                    "Decode avif image failed".to_owned(),
                                )
                            })?
                    ...
                    image::load_from_memory_with_format(input_buf, image_format).map_err(|err| {
                        Error::new(Status::InvalidArg, format!("Decode image failed {err}"))
                    })?
                };
            ...
            if let Some(orientation) = orientation {
                match orientation? {
                    ...
                    Orientation::MirrorHorizontalAndRotate90Cw => {
                        meta.image = meta.image.flipv().rotate270()
                    }
            ...
            match fit.unwrap_or_default() {
                ...
                ResizeFit::Cover => {
                    meta.image = meta.image.resize_to_fill(
                        width,
                        height.unwrap_or(
                            ((width as f32 / raw_width as f32) * (raw_height as f32)) as u32,
                        ),
    ...
    fn resolve(&mut self, env: Env, output: Self::Output) -> Result<Self::JsValue> {
        match output {
            EncodeOutput::Raw(buf, size) => unsafe {
                env.create_buffer_with_borrowed_data(buf, size, buf, move |buf_ptr, _env| {

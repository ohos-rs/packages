...
pub fn fast_resize(img: &DynamicImage, options: FastResizeOptions) -> Result<fr::Image> {
    ...
    let mut src_image =
        fr::Image::from_slice_u8(width, height, rgba8.as_mut(), fr::PixelType::U8x4)
    ...
    let mut dst_height = NonZeroU32::new(options.height.unwrap_or_else(|| {
        (options.width as f32 / img.width() as f32 * img.height() as f32) as u32
    }))
    .ok_or_else(|| {
        Error::new(
            Status::InvalidArg,
